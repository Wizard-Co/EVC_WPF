...
namespace WizMes_EVC
{
    ...
    /// </summary>
    public partial class PlusFinderWindow : Window
    {
        ...
        public void fillGrid(TextBox txtCountry, string text)
        {
            ...
            h1.Header = "키";
            h1.Binding = new Binding("key");
            h1.Width = 120;

            DataGridTextColumn h2 = new DataGridTextColumn();
            h2.Header = "값";
            h2.Binding = new Binding("value");
            h2.Width = new DataGridLength(1, DataGridLengthUnitType.Star);

            mDataGrid.Columns.Add(h1);
            mDataGrid.Columns.Add(h2);
        ...
        public void getDBGrid(int large, string mmiddle)
        {
            string sql = "";
            string dOrderByStr = "";
            ...
            DataSet ds = DataStore.Instance.QueryToDataSet(sql + dOrderByStr);
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                else
                {
                    ...
                    foreach (DataRow item in drc)
                    {
                        ...
                        var plusFinderView = new PlusFinderView()
                        {
                            key = item[0] as string,
                            value = item[1] as string,
                        };
                        mDataGrid.Items.Add(plusFinderView);
                    }
                }
        ...
        {
            Console.WriteLine(((DataGrid)sender).SelectedItem.ToString());
            PlusFinderView pfv = ((DataGrid)sender).SelectedItem as PlusFinderView;
            txtCountry.Text = pfv.value;
            this.Close();
        }
    }
}
